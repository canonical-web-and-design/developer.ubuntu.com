...
DEFAULT_NAVIGATION_OPTIONS = {"nesting_limit": 3}
...
class Sitemap:
        ...
        for config_item in config:
            ...
            if "_path" in config_item:
                current_node_path = "{current}/{node}".format(
                    current=current_path, node=config_item["_path"]
                )
    ...
    def build_navigation(self, current_path=None):
        ...
        config_path = os.path.join(
            template_path, "includes", "navigation.yaml"
        )
        config = []
        ...
        options["root_path"] = "/{path}".format(path=root_path_key or "")

        sorted_tree = self._populate_navigation(
            config_items, sitemap, options=options
        )
